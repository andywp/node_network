...
class NodeNetwork {
  constructor(token, proxyUrl = null) {
    this.apiBaseUrl = 'https://api.optimai.network';
  ...
  async ping() {
    try {
      ...
      if (currentTime - this.lastPingTimestamp < 1000) {
        await new Promise(resolve => setTimeout(resolve, 1000 - (currentTime - this.lastPingTimestamp)));
      }

      const response = await this.makeRequest('GET', '/users/balance');
      ...
      console.log(chalk.white(`📡 Balance: ${response.data.balance}`));

      /**rewards */

      const rewards= await this.makeRequest('GET', '/dashboard/node-operator-rewards');
      console.log(chalk.red(`💾 Uptime Hours:`+rewards.data.uptime_hours));
      const profile= await this.makeRequest('GET', '/auth/me?platforms=all');
      console.log(chalk.white(`💾 Akun Name: ${profile.data.user.display_name}`));
      console.log(chalk.green(`------------tinggu dilit--------------------`)); 

      return response.data;
    } catch (error) {
      console.log(chalk.red(`etsss  [ERR] ${error.message}`));
      throw error;
    }
  }
}
...
class MultiAccountPinger {
  ...
  async processPing(account) {
    const pinger = new NodeNetwork(account.token, account.proxy);
